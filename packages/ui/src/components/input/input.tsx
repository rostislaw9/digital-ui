...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(
  ...
  return (
    <input
      ...
      // prettier-ignore
      className={cn("flex h-8 w-full rounded-md border bg-surface px-3 py-1.5 text-base sm:text-sm text-foreground placeholder:text-foreground-subtle transition-[border-color,box-shadow,background-color] duration-[var(--duration-fast)] ease-[var(--ease-standard)] focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-40 border-border hover:border-border-strong focus-visible:shadow-focus", invalid && ["border-error hover:border-error", "focus-visible:shadow-focus-error"], className)}
      {...props}
    />
  );
});
